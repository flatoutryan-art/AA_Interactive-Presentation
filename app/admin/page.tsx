...
async function parseOfferbook(file: File): Promise<ParsedOfferbook> {
  ...
  const totalSavingsDI = num(sDI, 'D43', 1, 2);
  const npvSavings     = num(sDI, 'D45', 1, 2);
  ...
  const touPeakHSWeps   = num(sDI, 'C28', 1, 4);
  const touStdHSWeps    = num(sDI, 'C29', 1, 4);
  const touOPHSWeps     = num(sDI, 'C30', 1, 4);
  const touPeakLSWeps   = num(sDI, 'C31', 1, 4);
  const touStdLSWeps    = num(sDI, 'C32', 1, 4);
  const touOPLSWeps     = num(sDI, 'C33', 1, 4);

  // DEAL IOs D64 = GHG Savings tCO2e/yr
  const ghgSavings = num(sDI, 'D64', 1, 2);

  // DEAL IOs D47 = Value Return HR (decimal) — same for all tenures
  const valueReturnHR = num(sDI, 'D47', 1, 4);
