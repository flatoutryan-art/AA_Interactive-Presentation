...
type ParsedOfferbook = {
  ...
  tariff_5yr?: string; tariff_10yr?: string; tariff_15yr?: string;
  eskom_tariff?: string;
  savings_5yr?: string; savings_10yr?: string; savings_15yr?: string;
  forex_exposure_pct?: string; volume_guarantee_pct?: string;
  credit_support_5yr?: string; credit_support_10yr?: string; credit_support_15yr?: string;
...
async function parseOfferbook(file: File): Promise<ParsedOfferbook> {
  ...
  // Returns a rounded numeric string; multiplies by `scale` if provided
  const num = (sheet: XLSX.WorkSheet | null, ref: string, scale = 1, dp = 4): string => {
    const v = cell(sheet, ref);
    const n = parseFloat(v);
    if (isNaN(n)) return '';
    const result = n * scale;
    return parseFloat(result.toFixed(dp)).toString();
  };

  // ── Sheet references — exact names from MasterRetail_Book ────────────────
  // Sheet names: 'Buyer Input', 'DEAL IOs', 'Pricing Model',
  //              'Financial Projection', 'Energy volumes', 'Assumptions'
  ...
  // ── CLIENT IDENTITY ───────────────────────────────────────────────────────
  // 'Buyer Input'  C7  = Client / Buyer name
  const clientName = cell(sBI, 'C7');
  const slug       = clientName ? slugify(clientName) : '';

  // ── TECHNICAL ─────────────────────────────────────────────────────────────
  // 'DEAL IOs'  D59 = Apollo Green Supply MWh/yr (contracted supply)
  // 'DEAL IOs'  D60 = Customer Electrical Load MWh/yr
  // 'DEAL IOs'  D7  = Target green coverage (decimal → ×100 for %)
  // 'DEAL IOs'  D11 = Volume / supply guarantee (decimal → ×100 for %)
  const contractMwh     = num(sDI, 'D59', 1, 0);
  const customerLoadMwh = num(sDI, 'D60', 1, 0);
  const greenCoverage   = num(sDI, 'D7',  100, 1);   // e.g. 0.70 → 70
  const volGuarantee    = num(sDI, 'D11', 100, 1);   // e.g. 0.70 → 70

  // ── TARIFFS ───────────────────────────────────────────────────────────────
  // Apollo offerbook is single-term — the active tariff lives at DEAL IOs D43.
  // We populate the matching term slot; the other two are left blank so the
  // proposal viewer only shows the one active term (partial-term feature).
  //
  // 'DEAL IOs'      D42 = Contract term in years (5, 10, or 15)
  // 'DEAL IOs'      D43 = Apollo weighted-average tariff R/kWh (1 Apr 2026)
  // 'Pricing Model' J49 = Eskom WEPS weighted average R/kWh (col J = NLA WEPS WA)
  const termYears    = parseInt(num(sDI, 'D42', 1, 0)) || 0;
  const apolloTariff = num(sDI,  'D43', 1, 4);
  const eskomTariff  = num(sPM,  'J49', 1, 4);  // col J = Eskom WEPS NLA weighted avg

  // Place the tariff only in the column that matches the contract term
  const tariff5yr  = termYears === 5  ? apolloTariff : '';
  const tariff10yr = termYears === 10 ? apolloTariff : '';
  const tariff15yr = termYears === 15 ? apolloTariff : '';

  // ── CUMULATIVE SAVINGS ────────────────────────────────────────────────────
  // 'Financial Projection' row 26:
  //   col N (index 13) = cumulative saving at year  5  (R mill)
  //   col S (index 18) = cumulative saving at year 10  (R mill)
  //   col X (index 23) = cumulative saving at year 15  (R mill)
  // We only populate the column matching the active term (same partial-term logic).
  const savings5yr  = termYears === 5  ? num(sFP, 'N26', 1, 2) : '';
  const savings10yr = termYears === 10 ? num(sFP, 'S26', 1, 2) : '';
  const savings15yr = termYears === 15 ? num(sFP, 'X26', 1, 2) : '';

  // ── COMMERCIAL ────────────────────────────────────────────────────────────
  // 'Assumptions'  C20 = FX / Forex exposure (decimal → ×100)
  // 'DEAL IOs'     D52 = Buyer credit support R mill
  const forexPct      = num(sAS, 'C20', 100, 1);
  const creditSupport = num(sDI, 'D52', 1, 2);

  // ── ESCALATION ────────────────────────────────────────────────────────────
  // 'Assumptions'  C8  = CPI  (decimal → ×100)
  // 'Assumptions'  C6  = Eskom long-term escalation (decimal → ×100)
  const cpi         = num(sAS, 'C8', 100, 1);
  const eskomEscPct = num(sAS, 'C6', 100, 1);

  // ── MONTHLY PROFILES ─────────────────────────────────────────────────────
  // 'Energy volumes'
  //   Row 41, cols E–P = Monthly Apollo supply in kWh  → ÷1000 = MWh
  //   Row 11, cols E–P = Monthly customer load in kWh  → ÷1000 = MWh
  ...
  MONTH_KEYS.forEach((k, i) => {
    ...
    // kWh → MWh: divide by 1000, round to 1 dp
    const supplyKwh = parseFloat(cell(sEV, `${col}41`) || '0');
    const loadKwh   = parseFloat(cell(sEV, `${col}11`) || '0');
    monthlySupply[k] = (Math.round(supplyKwh / 100) / 10).toString();  // 1 dp MWh
    monthlyLoad[k]   = (Math.round(loadKwh   / 100) / 10).toString();
  });

  console.log('[Apollo Uploader] Parsed cell map:', {
    clientName, slug, termYears, apolloTariff, eskomTariff,
    contractMwh, customerLoadMwh, greenCoverage,
    savings5yr, savings10yr, savings15yr,
  ...
  return {
    ...
    customer_load_mwh:    customerLoadMwh,
    green_coverage_pct:   greenCoverage   || '70',
    tariff_5yr:           tariff5yr,
    tariff_10yr:          tariff10yr,
    tariff_15yr:          tariff15yr,
    eskom_tariff:         eskomTariff     || '1.49',
    savings_5yr:          savings5yr,
    savings_10yr:         savings10yr,
    savings_15yr:         savings15yr,
    forex_exposure_pct:   forexPct        || '55',
    volume_guarantee_pct: volGuarantee    || '70',
    credit_support_5yr:   termYears === 5  ? creditSupport : '',
    credit_support_10yr:  termYears === 10 ? creditSupport : '',
    credit_support_15yr:  termYears === 15 ? creditSupport : '',
    escalation_cpi:       cpi             || '4.5',
    eskom_escalation:     eskomEscPct     || '6.0',
    monthlySupply,
    monthlyLoad,
...
export default function AdminPage() {
  ...
  // Excel upload handler — MERGE mode
  // Each upload only writes fields that are non-empty in the parsed data.
  // Upload the 5yr, 10yr, and 15yr versions one after another —
  // each fills only its own term without wiping the others.
  const handleParsed = useCallback((data: ParsedOfferbook) => {
    // ── Shared fields — always overwrite (same across all term variants) ───
    ...
    // ── Term-specific fields — only write when this upload contains that term
    // The parser returns '' for terms not in the file, so existing data is preserved.
  ...
  return (
    <div className="min-h-screen bg-charcoal py-12 px-4 font-sans">
      <div className="max-w-3xl mx-auto">
        ...
        {/* Terms-loaded status strip — shows which term variants have been uploaded */}
        <div className="mb-3 flex items-center gap-2 flex-wrap">
          <span className="text-[11px] font-bold uppercase tracking-widest text-muted">Terms loaded:</span>
          {([5,10,15] as const).map(yr => {
            const loaded = yr===5?!!t5 : yr===10?!!t10 : !!t15;
            return (
              <span key={yr} className={`px-3 py-1 rounded-full text-xs font-bold border transition-all ${
                loaded
                  ? 'bg-green/15 border-green/50 text-green'
                  : 'bg-forest border-border text-border'
              }`}>
                {yr}yr {loaded ? '✓' : '—'}
              </span>
            );
          })}
          <span className="text-[11px] text-dim ml-1">Upload each term variant separately to populate all three.</span>
        </div>
