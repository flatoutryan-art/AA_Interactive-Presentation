...
type ParsedOfferbook = {
  ...
  value_return_hr?: string;
  ghg_savings?: string;
  tou_peak_hs_apollo?: string; tou_std_hs_apollo?: string; tou_op_hs_apollo?: string;
  tou_peak_ls_apollo?: string; tou_std_ls_apollo?: string; tou_op_ls_apollo?: string;
  tou_peak_hs_weps?: string;   tou_std_hs_weps?: string;   tou_op_hs_weps?: string;
  tou_peak_ls_weps?: string;   tou_std_ls_weps?: string;   tou_op_ls_weps?: string;
...
async function parseOfferbook(file: File): Promise<ParsedOfferbook> {
  ...
  // ── Safe cell reader — returns empty string if cell missing/null ──────────
  const cell = (sheet: XLSX.WorkSheet | null, ref: string): string => {
    if (!sheet) return '';
    const c = sheet[ref];
    if (!c || c.v === null || c.v === undefined) return '';
    if (c.t === 'd') {
      try { return new Date(c.v as string | number).toISOString().split('T')[0]; }
      catch { /* fall through */ }
    }
    return String(c.v).trim();
  };

  // scale: multiply raw value (e.g. 100 to convert 0.70 → 70%)
  // dp: decimal places to round to
  const num = (sheet: XLSX.WorkSheet | null, ref: string, scale = 1, dp = 4): string => {
    const raw = parseFloat(cell(sheet, ref));
    if (isNaN(raw)) return '';
    return parseFloat((raw * scale).toFixed(dp)).toString();
  };
  ...
  // ══════════════════════════════════════════════════════════════════════════
  // VERIFIED CELL MAP — confirmed against MasterRetail_Book v75.1
  // Labels are always in col B; values in col C or D depending on section.
  // Run with browser console open (F12) after upload to verify new versions.
  // ══════════════════════════════════════════════════════════════════════════
  ...
  // ── CONTRACT TERM ─────────────────────────────────────────────────────────
  // DEAL IOs  C5  = Contract Length (years)  [label in B5]
  // Also mirrored at D40 with label in C40 — C5 is primary input cell
  const termYears = parseInt(cell(sDI, 'C5') || cell(sDI, 'D40') || '0') || 0;

  // ── TARIFFS ───────────────────────────────────────────────────────────────
  // DEAL IOs  D41 = Apollo 1 April 2026 Tariff R/kWh  [label in B41, unit C41]
  // DEAL IOs  D56 = WEPS Credit R/kWh                 [label in B56, unit C56]
  const apolloTariff = num(sDI, 'D41', 1, 4);
  const eskomTariff  = num(sDI, 'D56', 1, 4);

  // ── TECHNICAL ─────────────────────────────────────────────────────────────
  // DEAL IOs  D57 = Customer Electrical Load MWh/yr   [label B57, unit C57]
  // DEAL IOs  D60 = Contracted Volume MWh/yr          [label B60, unit C60]
  // DEAL IOs  D62 = ToP Coverage % (decimal)          [label B62, unit C62]
  // DEAL IOs  D8  = Supply Guarantee (decimal)        [label B8]
  const customerLoad = num(sDI, 'D57', 1, 0);
  const contractMwh  = num(sDI, 'D60', 1, 0);
  const greenCov     = num(sDI, 'D62', 100, 1);
  const volGuar      = num(sDI, 'D8',  100, 1);

  // ── SAVINGS & FINANCIALS ──────────────────────────────────────────────────
  // DEAL IOs  D43 = Total Contract Savings MR R mill  [label B43]
  // DEAL IOs  D45 = NPV of Savings MR R mill          [label B45]
  // DEAL IOs  D47 = Value Return MR (decimal)         [label B47]
  // DEAL IOs  D50 = Buyer Credit Support R mill       [label B50]
  const totalSavingsDI = num(sDI, 'D44', 1, 2);  // D44 = Total Contract Savings HR
  const npvSavings     = num(sDI, 'D46', 1, 2);  // D46 = NPV of Savings HR
  const creditSup      = num(sDI, 'D50', 1, 2);

  // Financial Projection row 26 = Cumulative Savings (R mill) MR
  // COD = FY2029 = col G. Contract year cols (0-indexed):
  //   yr1=J(9), yr2=K(10), yr3=L(11), yr4=M(12), yr5=N(13),
  //   yr6=O(14), yr7=P(15), yr8=Q(16), yr9=R(17), yr10=S(18),
  //   yr11=T(19), yr12=U(20), yr13=V(21), yr14=W(22), yr15=X(23)
  const savingsMap: Record<number, string> = {
    ...
    10: num(sFP, 'S26', 1, 2),
    15: num(sFP, 'X26', 1, 2),
    20: num(sFP, 'X26', 1, 2),  // 20yr not in model — use 15yr max
  };

  // Year 1 annual saving = FP row 25 col J (first contract year)
  // FP row 25 col E = Total Contract Savings (same as DI D43, cross-check)
  const year1Savings = num(sFP, 'J25', 1, 2);

  // ── COMMERCIAL ────────────────────────────────────────────────────────────
  // Assumptions col C (values) col B (labels):
  //   C6  = Eskom Long Term Forecast (decimal)
  //   C8  = CPI (decimal)
  //   C20 = FX Exposure (decimal)
  const eskomEsc = num(sAS, 'C6',  100, 1);
  const cpi      = num(sAS, 'C8',  100, 1);
  const forexPct = num(sAS, 'C20', 100, 1);

  // ── ACTUAL TOU TARIFFS ───────────────────────────────────────────────────────
  // DEAL IOs E19:E24 = Apollo TOU tariffs [R/kWh] per period
  // DEAL IOs C28:C33 = Eskom WEPS tariffs [R/kWh] per period
  // Row order: Peak HS, Std HS, OffPk HS, Peak LS, Std LS, OffPk LS
  const touPeakHSApollo = num(sDI, 'E19', 1, 4);
  const touStdHSApollo  = num(sDI, 'E20', 1, 4);
  const touOPHSApollo   = num(sDI, 'E21', 1, 4);
  const touPeakLSApollo = num(sDI, 'E22', 1, 4);
  const touStdLSApollo  = num(sDI, 'E23', 1, 4);
  const touOPLSApollo   = num(sDI, 'E24', 1, 4);
  const touPeakHSWeps   = num(sDI, 'D28', 1, 4);
  const touStdHSWeps    = num(sDI, 'D29', 1, 4);
  const touOPHSWeps     = num(sDI, 'D30', 1, 4);
  const touPeakLSWeps   = num(sDI, 'D31', 1, 4);
  const touStdLSWeps    = num(sDI, 'D32', 1, 4);
  const touOPLSWeps     = num(sDI, 'D33', 1, 4);

  // DEAL IOs D64 = GHG Savings tCO2e/yr
  const ghgSavings = num(sDI, 'D64', 1, 2);

  // DEAL IOs D48 = Value Return HR (decimal)
  const valueReturnHR = num(sDI, 'D48', 1, 4);  // D48 = Value Return HR

  // ── TOU ENERGY ALLOCATION ─────────────────────────────────────────────────
  // Pricing Model rows 53-58, col D = fraction of annual contracted supply
  //   Row 53 col D = Peak HS fraction
  //   Row 54 col D = Standard HS fraction
  //   Row 55 col D = Off-Peak HS fraction
  //   Row 56 col D = Peak LS fraction
  //   Row 57 col D = Standard LS fraction
  //   Row 58 col D = Off-Peak LS fraction
  // Sum HS+LS for each TOU category to get annual %
  const peakHS = parseFloat(cell(sPM, 'D53') || '0');
  const stdHS  = parseFloat(cell(sPM, 'D54') || '0');
  const opHS   = parseFloat(cell(sPM, 'D55') || '0');
  const peakLS = parseFloat(cell(sPM, 'D56') || '0');
  const stdLS  = parseFloat(cell(sPM, 'D57') || '0');
  const opLS   = parseFloat(cell(sPM, 'D58') || '0');

  const touPeak     = (((peakHS + peakLS)) * 100).toFixed(1);
  const touStandard = (((stdHS  + stdLS))  * 100).toFixed(1);
  const touOffpeak  = (((opHS   + opLS))   * 100).toFixed(1);

  // ── MONTHLY SUPPLY & LOAD ─────────────────────────────────────────────────
  // Energy volumes sheet:
  //   Row 41 cols E–P = Total Monthly contracted supply [kWh] → ÷1000 = MWh
  //   Row 11 cols E–P = Total Monthly customer load     [kWh] → ÷1000 = MWh
  ...
  // ── LOG for calibration ───────────────────────────────────────────────────
  console.log('[Apollo Uploader] Parsed:', {
    clientName, slug, termYears,
    apolloTariff, eskomTariff,
    contractMwh, customerLoad, greenCov, volGuar,
    year1Savings, totalSavingsDI, npvSavings, creditSup,
    cpi, eskomEsc, forexPct,
  ...
  // ── RETURN ────────────────────────────────────────────────────────────────
  // Term-specific fields (tariff, savings, credit) only populate for the
  // active term in this file. This enables multi-upload workflow:
  // upload 5yr → fills t5/s5/c5 only, upload 10yr next → fills t10/s10/c10.
  return {
    ...
    ghg_savings:          ghgSavings,
    value_return_hr:      valueReturnHR,
    tou_peak_hs_apollo:   touPeakHSApollo,
    tou_std_hs_apollo:    touStdHSApollo,
    tou_op_hs_apollo:     touOPHSApollo,
    tou_peak_ls_apollo:   touPeakLSApollo,
    tou_std_ls_apollo:    touStdLSApollo,
    tou_op_ls_apollo:     touOPLSApollo,
    tou_peak_hs_weps:     touPeakHSWeps,
    tou_std_hs_weps:      touStdHSWeps,
    tou_op_hs_weps:       touOPHSWeps,
    tou_peak_ls_weps:     touPeakLSWeps,
    tou_std_ls_weps:      touStdLSWeps,
    tou_op_ls_weps:       touOPLSWeps,
    // Term-specific — only populated for the active term in this upload
    ...
    savings_3yr:          termYears === 3  ? (savingsMap[3]  || totalSavingsDI) : '',
    savings_5yr:          termYears === 5  ? (savingsMap[5]  || totalSavingsDI) : '',
    savings_7yr:          termYears === 7  ? (savingsMap[7]  || totalSavingsDI) : '',
    savings_10yr:         termYears === 10 ? (savingsMap[10] || totalSavingsDI) : '',
    savings_15yr:         termYears === 15 ? (savingsMap[15] || totalSavingsDI) : '',
    savings_20yr:         termYears === 20 ? (savingsMap[20] || totalSavingsDI) : '',
...
export default function AdminPage() {
  ...
  // TOU allocation
  const [touPeak,     setTouPeak]     = useState('17');
  const [touStandard, setTouStandard] = useState('43');
  const [touOffpeak,  setTouOffpeak]  = useState('40');
  const [valueReturnHR,    setValueReturnHR]    = useState('');
  const [ghgSavingsDB,     setGhgSavingsDB]     = useState('');
  const [touPeakHSApollo,  setTouPeakHSApollo]  = useState('');
  const [touStdHSApollo,   setTouStdHSApollo]   = useState('');
  const [touOPHSApollo,    setTouOPHSApollo]    = useState('');
  const [touPeakLSApollo,  setTouPeakLSApollo]  = useState('');
  const [touStdLSApollo,   setTouStdLSApollo]   = useState('');
  const [touOPLSApollo,    setTouOPLSApollo]    = useState('');
  const [touPeakHSWeps,    setTouPeakHSWeps]    = useState('');
  const [touStdHSWeps,     setTouStdHSWeps]     = useState('');
  const [touOPHSWeps,      setTouOPHSWeps]      = useState('');
  const [touPeakLSWeps,    setTouPeakLSWeps]    = useState('');
  const [touStdLSWeps,     setTouStdLSWeps]     = useState('');
  const [touOPLSWeps,      setTouOPLSWeps]      = useState('');
  ...
  async function submit() {
    ...
    try {
      ...
      const p: Omit<Proposal,'id'|'created_at'|'carbon_savings'> = {
        ...
        tou_peak_pct:     Number(touPeak)     || 17,
        tou_standard_pct: Number(touStandard) || 43,
        tou_offpeak_pct:  Number(touOffpeak)  || 40,
        ghg_savings:           Number(ghgSavingsDB)    || 0,
        value_return_hr:       Number(valueReturnHR)   || 0,
        tou_peak_hs_apollo:    Number(touPeakHSApollo) || 0,
        tou_std_hs_apollo:     Number(touStdHSApollo)  || 0,
        tou_op_hs_apollo:      Number(touOPHSApollo)   || 0,
        tou_peak_ls_apollo:    Number(touPeakLSApollo) || 0,
        tou_std_ls_apollo:     Number(touStdLSApollo)  || 0,
        tou_op_ls_apollo:      Number(touOPLSApollo)   || 0,
        tou_peak_hs_weps:      Number(touPeakHSWeps)   || 0,
        tou_std_hs_weps:       Number(touStdHSWeps)    || 0,
        tou_op_hs_weps:        Number(touOPHSWeps)     || 0,
        tou_peak_ls_weps:      Number(touPeakLSWeps)   || 0,
        tou_std_ls_weps:       Number(touStdLSWeps)    || 0,
        tou_op_ls_weps:        Number(touOPLSWeps)     || 0,
