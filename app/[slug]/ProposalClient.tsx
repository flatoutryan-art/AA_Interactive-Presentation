  ...
  return (
    <div className="min-h-screen bg-charcoal text-offwhite font-sans">
      ...
      <div className="max-w-4xl mx-auto px-5">
        ...
        {/* ── TOU TARIFF SCHEDULE — moved here from Charts, actual DEAL IOs values ── */}
        <section className="py-14">
          <SHead eye="TOU Tariff Schedule [R/kWh]" title="Apollo vs Eskom — 1 April 2026" />
          {(() => {
            // Use actual stored TOU tariffs if available, else fall back to ratio approximations
            const hasTouData = p.touPeakHSApollo > 0;
            const rows = [
              {
                label: 'Peak (High Season)', season: 'HS',
                apolloRate: hasTouData ? p.touPeakHSApollo : baseTariff * 1.38,
                wepsRate:   hasTouData ? p.touPeakHSWeps   : p.eskom    * 1.40,
                mwh: touPeakMwh * 0.5, pct: p.touPeak * 0.5,
              },
              {
                label: 'Standard (High Season)', season: 'HS',
                apolloRate: hasTouData ? p.touStdHSApollo : baseTariff * 0.97,
                wepsRate:   hasTouData ? p.touStdHSWeps   : p.eskom    * 0.97,
                mwh: touStdMwh * 0.5, pct: p.touStd * 0.5,
              },
              {
                label: 'Off-Peak (High Season)', season: 'HS',
                apolloRate: hasTouData ? p.touOPHSApollo : baseTariff * 0.68,
                wepsRate:   hasTouData ? p.touOPHSWeps   : p.eskom    * 0.62,
                mwh: touOPMwh * 0.5, pct: p.touOP * 0.5,
              },
              {
                label: 'Peak (Low Season)', season: 'LS',
                apolloRate: hasTouData ? p.touPeakLSApollo : baseTariff * 0.58,
                wepsRate:   hasTouData ? p.touPeakLSWeps   : p.eskom    * 0.58,
                mwh: touPeakMwh * 0.5, pct: p.touPeak * 0.5,
              },
              {
                label: 'Standard (Low Season)', season: 'LS',
                apolloRate: hasTouData ? p.touStdLSApollo : baseTariff * 0.85,
                wepsRate:   hasTouData ? p.touStdLSWeps   : p.eskom    * 0.85,
                mwh: touStdMwh * 0.5, pct: p.touStd * 0.5,
              },
              {
                label: 'Off-Peak (Low Season)', season: 'LS',
                apolloRate: hasTouData ? p.touOPLSApollo : baseTariff * 0.68,
                wepsRate:   hasTouData ? p.touOPLSWeps   : p.eskom    * 0.62,
                mwh: touOPMwh * 0.5, pct: p.touOP * 0.5,
              },
            ];
            return (
              <div className="bg-white border border-gray-200 rounded-2xl shadow-sm overflow-hidden">
                  ...
                  <table className="w-full border-collapse" style={{fontSize:12}}>
                    <thead>
                      <tr className="bg-gray-50 border-b border-gray-200">
                        <th className="text-left py-3 px-4 text-[11px] font-bold uppercase text-gray-500 w-56">TOU Period</th>
                    ...
                    <tbody>
                      {rows.map((row, idx) => {
                        const saving    = row.wepsRate - row.apolloRate;
                        const savingPct = row.wepsRate > 0 ? (saving / row.wepsRate) * 100 : 0;
                        const isHS      = row.season === 'HS';
                        return (
                          <tr key={row.label} className={`border-b border-gray-100 hover:bg-gray-50 transition-colors ${isHS ? '' : 'bg-blue-50/20'}`}>
                            <td className={`py-3 px-4 font-medium ${isHS ? 'text-amber-700' : 'text-gray-600'}`}>{row.label}</td>
                            <td className="text-center py-3 px-3 text-gray-700">{fmt(row.mwh, 0)}</td>
                            <td className="text-center py-3 px-3 text-gray-500 text-xs">{fmt(row.pct, 1)}%</td>
                            <td className="text-center py-3 px-3 text-green font-semibold">{fmt(row.apolloRate)}</td>
                            <td className="text-center py-3 px-3 text-red-500">{fmt(row.wepsRate)}</td>
                      ...
                      <tr className="bg-green/5 border-t-2 border-green/20">
                        ...
                        <td className="text-center py-3 px-3 font-black text-green text-base">-{fmt(p.eskom - baseTariff)}</td>
                ...
                <div className="px-5 py-3 bg-gray-50 border-t border-gray-100">
                  <p className="text-gray-400 text-[11px]">
                    Apollo tariffs from 1 April 2026 (DEAL IOs E19–E25) · Eskom WEPS HR (DEAL IOs D28–D34) · High season = Jun–Aug · Low season = Sep–May · Apollo escalates at CPI annually.
                    {!hasTouData && ' TOU period rates are indicative — upload offerbook to populate exact values.'}
                  </p>
                </div>
              </div>
            );
