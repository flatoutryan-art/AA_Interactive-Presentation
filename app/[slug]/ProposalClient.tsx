...
export default function ProposalClient({ slug }: { slug: string }) {
  ...
  const p = {
    ...
    touPeak:    safe(proposal.tou_peak_pct)     || 17,
    touStd:     safe(proposal.tou_standard_pct) || 43,
    touOP:      safe(proposal.tou_offpeak_pct)  || 40,
    // Actual TOU tariffs from DEAL IOs E19:E25 (Apollo) and C28:C34 (WEPS)
    touPeakHSApollo:  safe(proposal.tou_peak_hs_apollo),
    touStdHSApollo:   safe(proposal.tou_std_hs_apollo),
    touOPHSApollo:    safe(proposal.tou_op_hs_apollo),
    touPeakLSApollo:  safe(proposal.tou_peak_ls_apollo),
    touStdLSApollo:   safe(proposal.tou_std_ls_apollo),
    touOPLSApollo:    safe(proposal.tou_op_ls_apollo),
    touPeakHSWeps:    safe(proposal.tou_peak_hs_weps),
    touStdHSWeps:     safe(proposal.tou_std_hs_weps),
    touOPHSWeps:      safe(proposal.tou_op_hs_weps),
    touPeakLSWeps:    safe(proposal.tou_peak_ls_weps),
    touStdLSWeps:     safe(proposal.tou_std_ls_weps),
    touOPLSWeps:      safe(proposal.tou_op_ls_weps),
    valueReturnHR:    safe(proposal.value_return_hr),
    ghgSavings:       safe(proposal.ghg_savings),      // DEAL IOs D64 tCO2e/yr
  };
  ...
  // GHG Savings: use DEAL IOs D64 (exact model value) if stored, else formula
  const ghgAnnual  = p.ghgSavings > 0 ? p.ghgSavings : generated * 0.94;
  ...
  // Year 1 saving — from Financial Projection J25, scaled by coverage ratio
  const year1Saving = p.s1 > 0 ? p.s1 * savingsRatio : annualSavingY1;
  // Total contract savings = adjSavings (scaled)
  const totalContractSavings = adjSavings;
  // Value return — use DB D48 if available (Value Return HR), else compute
  const valueReturn = p.valueReturnHR > 0
    ? p.valueReturnHR * 100
    : (() => { const c = baseTariff * generated * term * 1000 / 1_000_000; return c > 0 ? (totalContractSavings / c) * 100 : 0; })();
  // NPV — use DB value if available
  const npvSavings = p.npv > 0 ? p.npv : adjSavings * 0.72;
      ...
      <div className="max-w-4xl mx-auto px-5">
        ...
        <section className="py-14">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-3">
            ...
            <Stat label="Year 1 Savings"             value={`R${(year1Saving).toFixed(1)}m`}          sub="first contract year"          accent />
            <Stat label={`Total ${term}-yr Savings`} value={`R${(totalContractSavings).toFixed(1)}m`} sub="cumulative contract" />
          </div>

          {/* Row 2 — financial KPIs */}
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-3">
            <Stat label="Value Return"   value={`${fmt(valueReturn,1)}%`}      sub="Value Return HR"  accent />
            <Stat label="NPV Savings"    value={`R${npvSavings.toFixed(1)}m`}  sub="NPV of Savings HR" />
            <Stat label="Credit Support" value={`R${fmt(baseCredit,1)}m`}      sub="ZAR million"     accent />
            <Stat label="GHG Savings"    value={`${fmt(ghgAnnual,0)} tCO₂e`}  sub="per year" />
          </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            ...
            <Stat label="Eskom WEPS"      value={`R${fmt(p.eskom)}/kWh`} sub="weighted avg tariff" />
          </div>
        </section>

        <HR />

        {/* ── TOU TARIFF SCHEDULE — Direct from DEAL IOs E19:E25 (Apollo) & D28:D34 (WEPS HR) ── */}
        <section className="py-14">
          <SHead eye="TOU Tariff Schedule [R/kWh]" title="Apollo vs Eskom — 1 April 2026" />
          {(() => {
            // Direct DB values — no fallback ratios.
            // Apollo  = tou_*_apollo (DEAL IOs E19:E24, rows: PeakHS, StdHS, OPhs, PeakLS, StdLS, OPLS)
            // WEPS HR = tou_*_weps   (DEAL IOs D28:D33, same period order offset by 9 rows)
            // WA row  = baseTariff (Apollo C25/E25) vs p.eskom (WEPS D34)
            // Energy MWh per period = generated × allocation% (split equally HS/LS within each TOU)
            const hasData = p.touPeakHSApollo > 0 && p.touPeakHSWeps > 0;

            const rows = [
              { label:'Peak (High Season)',     season:'HS', apollo:p.touPeakHSApollo, weps:p.touPeakHSWeps, pct:p.touPeak * 0.5 },
              { label:'Standard (High Season)', season:'HS', apollo:p.touStdHSApollo,  weps:p.touStdHSWeps,  pct:p.touStd  * 0.5 },
              { label:'Off-Peak (High Season)', season:'HS', apollo:p.touOPHSApollo,   weps:p.touOPHSWeps,   pct:p.touOP   * 0.5 },
              { label:'Peak (Low Season)',      season:'LS', apollo:p.touPeakLSApollo, weps:p.touPeakLSWeps, pct:p.touPeak * 0.5 },
              { label:'Standard (Low Season)',  season:'LS', apollo:p.touStdLSApollo,  weps:p.touStdLSWeps,  pct:p.touStd  * 0.5 },
              { label:'Off-Peak (Low Season)',  season:'LS', apollo:p.touOPLSApollo,   weps:p.touOPLSWeps,   pct:p.touOP   * 0.5 },
            ];

            if (!hasData) {
              return (
                <div className="bg-white border border-gray-200 rounded-2xl p-8 text-center shadow-sm">
                  <p className="text-gray-400 text-sm mb-1">TOU tariff data not yet uploaded</p>
                  <p className="text-gray-300 text-xs">Re-upload the offerbook Excel file to populate exact TOU tariff values from DEAL IOs E19–E25 and D28–D34.</p>
                </div>
              );
            }
              ...
              <div className="bg-white border border-gray-200 rounded-2xl shadow-sm overflow-hidden">
                  ...
                  <table className="w-full border-collapse" style={{fontSize:12}}>
                    <thead>
                      <tr className="bg-gray-50 border-b border-gray-200">
                        <th className="text-left py-3 px-4 text-[11px] font-bold uppercase text-gray-500">TOU Period</th>
                        <th className="text-center py-3 px-3 text-[11px] font-bold uppercase text-gray-500">Energy (MWh)</th>
                        <th className="text-center py-3 px-3 text-[11px] font-bold uppercase text-gray-500">% Alloc.</th>
                        <th className="text-center py-3 px-3 text-[11px] font-bold uppercase text-green">Apollo {term}yr</th>
                        <th className="text-center py-3 px-3 text-[11px] font-bold uppercase text-red-500">Eskom WEPS HR</th>
                    ...
                    <tbody>
                      {rows.map(row => {
                        const mwh       = generated * (row.pct / 100);
                        const saving    = row.weps - row.apollo;
                        const savingPct = row.weps > 0 ? (saving / row.weps) * 100 : 0;
                        const isHS      = row.season === 'HS';
                        return (
                          <tr key={row.label} className={`border-b border-gray-100 hover:bg-gray-50 transition-colors ${!isHS ? 'bg-slate-50/40' : ''}`}>
                            <td className={`py-3 px-4 font-medium ${isHS ? 'text-amber-700' : 'text-gray-600'}`}>{row.label}</td>
                            <td className="text-center py-3 px-3 text-gray-700">{fmt(mwh, 0)}</td>
                            <td className="text-center py-3 px-3 text-gray-500 text-xs">{row.pct.toFixed(1)}%</td>
                            <td className="text-center py-3 px-3 text-green font-semibold">{fmt(row.apollo)}</td>
                            <td className="text-center py-3 px-3 text-red-500">{fmt(row.weps)}</td>
                            <td className={`text-center py-3 px-3 font-bold ${saving>0?'text-green':saving<0?'text-red-500':'text-gray-400'}`}>
                              {saving > 0 ? '-' : saving < 0 ? '+' : ''}{fmt(Math.abs(saving))}
                            </td>
                            <td className="text-center py-3 px-3 text-gray-500">{savingPct.toFixed(1)}%</td>
                          </tr>
                        );
                      })}
                      {/* Weighted Average row — from baseTariff (C25/E25) vs p.eskom (D34) */}
                      <tr className="bg-green/5 border-t-2 border-green/20">
                        <td className="py-3 px-4 font-black text-gray-900">Weighted Average</td>
                        <td className="text-center py-3 px-3 font-black text-green">{fmt(generated, 0)}</td>
                        <td className="text-center py-3 px-3 font-black text-gray-700">100%</td>
                        <td className="text-center py-3 px-3 font-black text-green text-base">{fmt(baseTariff)}</td>
                        <td className="text-center py-3 px-3 font-black text-red-500 text-base">{fmt(p.eskom)}</td>
                        <td className="text-center py-3 px-3 font-black text-green text-base">
                          -{fmt(p.eskom - baseTariff)}
                        </td>
                        <td className="text-center py-3 px-3 font-bold text-gray-700">
                          {p.eskom > 0 ? (((p.eskom - baseTariff) / p.eskom) * 100).toFixed(1) : '0.0'}%
                        </td>
                      </tr>
                    </tbody>
                  </table>
                ...
                <div className="px-5 py-3 bg-gray-50 border-t border-gray-100">
                  <p className="text-gray-400 text-[11px]">
                    Apollo tariffs: DEAL IOs E19–E25 · Eskom WEPS HR: DEAL IOs D28–D34 · 1 April 2026 ·
                    High season = Jun–Aug · Low season = Sep–May · Apollo escalates at CPI annually.
                  </p>
                </div>
              </div>
            );
        ...
        <section className="py-14">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            ...
            <div className="bg-forest border border-border rounded-2xl p-4">
              ...
              {/* D60 = contracted volume MWh/yr; scale by coverage slider */}
              <p className="text-xl font-black text-offwhite">{fmt(p.mwh > 0 ? p.mwh * (coveragePct / p.defaultCov) : generated, 0)}</p>
              <p className="text-xs text-dim">MWh / year · Deal IOs D60</p>
            </div>
            <div className="bg-forest border border-border rounded-2xl p-4">
            ...
            <div className="bg-forest border border-border rounded-2xl p-4">
              ...
              {/* D64 = GHG Savings tCO2e/yr from model; scale by coverage slider */}
              <p className="text-xl font-black text-mint">{fmt(ghgAnnual, 0)}</p>
              <p className="text-xs text-dim">tCO₂e / yr · Deal IOs D64</p>
            </div>
          </div>
          {coveragePct > 100 && (
        ...
        <section className="py-14">
          ...
          <div className="bg-forest border border-border rounded-2xl p-5">
            <div className="flex items-start gap-4">
              <div className="w-10 h-10 rounded-xl bg-green/20 border border-green/30 flex items-center justify-center flex-shrink-0 mt-0.5">
              ...
              <div className="flex-1">
                <div className="flex items-center justify-between gap-3 mb-2">
                  <p className="text-green font-black text-sm">iRECs Included — International Renewable Energy Certificates</p>
                  <span className="bg-green/15 border border-green/30 text-green text-[10px] font-bold uppercase tracking-widest px-2.5 py-1 rounded-full whitespace-nowrap flex-shrink-0">
                    Bundled in Tariff ✓
                  </span>
                </div>
                <p className="text-muted text-xs leading-relaxed mb-3">
                  Every MWh of Apollo supply is backed by a retired <strong className="text-offwhite">iREC</strong> — a globally recognised,
                  independently audited certificate proving renewable energy generation under the I-REC Standard.
                  iRECs satisfy <strong className="text-offwhite">GHG Protocol Scope 2</strong> market-based accounting and support CDP, TCFD,
                  and JSE ESG frameworks. For EU exporters, they provide CBAM compliance documentation.
                  Apollo manages all issuance, retirement, and reporting — <strong className="text-green">at no extra cost</strong>.
                </p>
                <div className="flex flex-wrap gap-4">
                  <div>
                    <p className="text-[10px] font-semibold uppercase tracking-wider text-muted">Annual iRECs</p>
                    <p className="text-green font-black text-xl leading-none">{fmt(generated, 0)}</p>
                    <p className="text-dim text-[10px]">certificates / yr</p>
                  </div>
                  <div className="w-px bg-border" />
                  <div>
                    <p className="text-[10px] font-semibold uppercase tracking-wider text-muted">{term}-yr Total</p>
                    <p className="text-green font-black text-xl leading-none">{fmt(generated * term, 0)}</p>
                    <p className="text-dim text-[10px]">iRECs over contract</p>
                  </div>
                  <div className="w-px bg-border" />
                  <div className="flex gap-2 flex-wrap">
                    {['ESG / TCFD','CDP / JSE','GHG Protocol Scope 2','EU CBAM Ready'].map(b => (
                      <span key={b} className="bg-green/10 border border-green/25 text-green text-[10px] font-semibold px-2.5 py-1 rounded-lg whitespace-nowrap">{b}</span>
                    ))}
                  </div>
                </div>
              </div>
        ...
        <Charts
          ...
          savings={{s3:p.s3,s5:p.s5,s7:p.s7,s10:p.s10,s15:p.s15,s20:p.s20}}
