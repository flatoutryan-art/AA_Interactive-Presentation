...
export default function ProposalClient({ slug }: { slug: string }) {
    ...
    supabase
      ...
      .then(({ data, error }) => {
        if (error || !data) {
          ...
          // Set initial term to the first available active term (e.g. 10 for a 10yr-only proposal)
  ...
  // ── Resolve active terms — fall back to all three if column missing ────────
  const activeTerms: number[] = proposal.active_terms?.length
    ? proposal.active_terms
    : [5, 10, 15];
  ...
  //
  // Economic model:
  //   - You pay Apollo for ALL generated MWh at apolloRate (no matter what)
  //   - You save Eskom cost only on CONSUMED (effective) MWh — capped at load
  //   - Spilled MWh cost apolloRate with ZERO Eskom offset → net cost not saving
  //
  // Annual saving = (effective × eskomRate) − (generated × apolloRate)
  //
  // Ratio logic:
  //   We scale the DB baseSavings (which is TOU-weighted from the financial model)
  //   using the ratio of current-coverage Y1 saving vs contracted-default Y1 saving.
  //   The denominator always uses the CONTRACTED default coverage (not slider value)
  //   so the ratio is stable and consistent regardless of where the slider sits.
  //
  //   Below 100% coverage: ratio < 1 → savings grow linearly with coverage
  //   At 100% coverage:    ratio peaks (all load is displaced, zero spill)
  //   Above 100% coverage: each extra MWh costs apolloRate, drops ratio sharply —
  //   this IS economically correct because Apollo bills on generation, not consumption.
  //
  // spill_penalty_note: exposed to UI so the slider can show an explanatory callout.
  ...
  // Annual R mill cost of spilled energy (Apollo charge with no Eskom offset)
  // Used in the UI to show an explicit spill penalty callout above 100% coverage
  const spillPenaltyMill = spillMwh * baseTariff * 1000 / 1_000_000;
  // Savings at 100% coverage (peak) — used as reference point in the UI
  const peakGenMwh       = p.load > 0 ? p.load : p.mwh;
  const peakAnnualY1     = ((peakGenMwh * p.eskom) - (peakGenMwh * baseTariff)) * 1000 / 1_000_000;
  const peakSavingsRatio = defaultAnnualY1 > 0 ? peakAnnualY1 / defaultAnnualY1 : 1;
  const peakAdjSavings   = baseSavings * peakSavingsRatio;
  ...
  const monthlyChartData = MONTH_LABELS.map((month, i) => {
    ...
    // Stacked chart: supply = consumed portion (capped at load).
    // spill stacks on top. supply + spill = total Apollo generation.
    return {
      month,
      supply: parseFloat(Math.min(rawSupply, loadVal).toFixed(0)),
      load:   parseFloat(loadVal.toFixed(0)),
      spill:  parseFloat(spill.toFixed(0)),
    };
  });
  ...
  // Only include tariff bars for active terms
  const tariffBars = [
  ...
  return (
    <div className="min-h-screen bg-charcoal text-offwhite font-sans">
      ...
      {/* ── STICKY TERM BAR ──────────────────────────────────────────────── */}
      {/*
        * Only renders term buttons for active terms.
        * If proposal is single-term, the pill is shown as a static label
        * rather than a clickable group — no point offering a "choice" of one.
      */}
      ...
      <div className="max-w-4xl mx-auto px-5">
        ...
        <section className="py-14">
          ...
          {/* Spill penalty callout — only visible above 100% coverage */}
          {coveragePct > 100 && (
            <div className="mt-4 bg-gold/8 border border-gold/40 rounded-xl p-4 flex gap-3 items-start">
              ...
              <div>
                <p className="text-gold text-xs font-bold uppercase tracking-widest mb-1">
                  Spillage Savings Impact — {fmt(spillPct, 1)}% of supply exceeds load
                </p>
                <p className="text-muted text-xs leading-relaxed">
                  At {coveragePct}% coverage, <strong className="text-offwhite">{fmt(spillMwh, 0)} MWh/yr</strong> of
                  Apollo supply exceeds your electrical load. Apollo bills on all generated MWh
                  at R{fmt(baseTariff)}/kWh — spilled energy has no offsetting Eskom saving.
                  This costs approximately <strong className="text-gold">R{fmt(spillPenaltyMill, 1)}m/yr</strong> in
                  unrecovered Apollo charges, reducing your net saving to <strong className="text-green">{fmtMill(adjSavings)}</strong> vs
                  a peak of <strong className="text-offwhite">{fmtMill(peakAdjSavings)}</strong> at 100% coverage.
                </p>
                <p className="text-gold/60 text-[11px] mt-1.5">
                  Tip: reduce coverage to ≤100% to maximise financial savings, or increase to ≥130% for EU CBAM compliance.
                </p>
              </div>
            </div>
          )}
