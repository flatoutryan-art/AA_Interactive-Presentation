      ...
      <section className="py-14">
        ...
        <div className="grid md:grid-cols-2 gap-5 mb-5">
          ...
          <div className="bg-white border border-gray-200 rounded-2xl p-5 shadow-sm flex flex-col gap-4">
            ...
            {/* Indicative savings block — computed from traj which uses selected escalation */}
            {(() => {
              const indicativeSavings = parseFloat(traj.reduce((s, r) => s + Math.max(0, r.annual), 0).toFixed(1));
              const diff    = parseFloat((indicativeSavings - adjSavings).toFixed(1));
              const diffPct = adjSavings > 0 ? Math.round((diff / adjSavings) * 100) : 0;
              const isHigh  = eskomEscPct > 6;
              const isLow   = eskomEscPct < 6;
              return (
                <div className={`rounded-xl p-4 border ${isHigh ? 'bg-green/5 border-green/30' : isLow ? 'bg-amber-50 border-amber-200' : 'bg-gray-50 border-gray-200'}`}>
                  <div className="flex items-start justify-between gap-2 mb-2">
                    <div>
                      <p className={`text-[10px] font-bold uppercase tracking-widest mb-0.5 ${isHigh ? 'text-green' : isLow ? 'text-amber-600' : 'text-gray-500'}`}>
                        Possible Savings @ {eskomEscPct}% Eskom
                      </p>
                      <p className={`text-2xl font-black leading-none ${isHigh ? 'text-green' : isLow ? 'text-amber-700' : 'text-gray-700'}`}>
                        R{indicativeSavings.toFixed(1)}m
                      </p>
                      <p className="text-gray-400 text-[10px] mt-0.5">cumulative over {traj.length}-year term</p>
                    </div>
                    {Math.abs(diff) >= 0.1 && (
                      <div className={`text-right flex-shrink-0 ${isHigh ? 'text-green' : isLow ? 'text-amber-600' : 'text-gray-400'}`}>
                        <p className="text-lg font-black leading-none">{isHigh ? '+' : ''}{diff.toFixed(1)}m</p>
                        <p className="text-[10px] font-semibold">{isHigh ? '+' : ''}{diffPct}% vs base</p>
                      </div>
                    )}
                  </div>
                  <p className="text-gray-500 text-[10px] leading-relaxed mb-2">
                    {isHigh
                      ? `At ${eskomEscPct}% p.a. the tariff gap widens — Apollo becomes progressively cheaper vs Eskom grid over the contract life.`
                      : isLow
                      ? `At ${eskomEscPct}% p.a. the advantage grows more slowly — Apollo is still cheaper, just less dramatically so.`
                      : `Base scenario: Eskom at ${eskomEscPct}% p.a. vs Apollo at CPI ${cpi}% p.a.`}
                  </p>
