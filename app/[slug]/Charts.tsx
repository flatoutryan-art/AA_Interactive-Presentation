      ...
      <section className="py-14">
        ...
        <Card title="Apollo Wheeled Supply vs Electrical Load [MWh / month]">
          <p className="text-muted text-xs mb-3 -mt-2">
            ...
            {spillMwh > 0 && (
              <span className="text-gold"> Gold fill stacked on top = spillage (supply exceeds load).</span>
            )}
          </p>
          <ResponsiveContainer width="100%" height={290}>
            {/*
              Stacked AreaChart — supply (capped at load) + spill stack on top of each other.
              The combined green+gold area will always sit AT or ABOVE the blue load line,
              which correctly communicates that Apollo supply >= load whenever spillage exists.

              Data shape expected:
                supply = Math.min(rawSupply, load)  — the portion actually consumed
                spill  = Math.max(0, rawSupply - load) — the excess stacked on top
              Both use stackId="apollo" so Recharts adds them vertically.
            */}
            <ComposedChart data={monthlyChartData} margin={{ top: 8, right: 8, left: 0, bottom: 0 }}>
              ...
              {/* Green base — supply consumed (capped at load). Stacked. */}
              <Area
                type="monotone"
                dataKey="supply"
                stackId="apollo"
              ...
              {/* Gold top — spillage stacked on top of supply, only when present */}
              {spillMwh > 0 && (
                <Area
                  type="monotone"
                  dataKey="spill"
              ...
              {/* Blue load line — drawn last so it renders on top of the fills */}
              <Line
                type="monotone"
                dataKey="load"
