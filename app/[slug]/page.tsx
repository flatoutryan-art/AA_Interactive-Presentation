...
export default function ProposalPage({ params }: { params: { slug: string } }) {
  ...
  // coveragePct: the slider value — green coverage as a whole-number percentage.
  // Defaults to null until data loads, then set to proposal.green_coverage_pct.
  // At default (e.g. 70): generated = customerLoad × 70% = 14,840 MWh
  // coveragePct displayed on slider == Green Coverage card — always identical.
    ...
    supabase
      ...
      .then(({ data, error }) => {
        if (error || !data) {
          ...
          // Default = the contracted green coverage (e.g. 70)
  ...
  const p = {
    ...
    defaultCov: safe(proposal.green_coverage_pct) || 70,  // contracted default
  ...
  // ── Coverage-derived — single source of truth ─────────────────────────────
  // generated MWh = customerLoad × coveragePct%
  // This means coverage card and slider always show the same number.
  const fraction    = coveragePct / 100;                        // e.g. 0.70
  const generated   = p.load > 0 ? p.load * fraction           // 21208 × 0.70 = 14,846
                    : p.mwh * fraction;                         // fallback if no load data
  ...
  // Savings scale proportionally to coverage relative to the contracted default
  const adjSavings  = baseSavings * (coveragePct / p.defaultCov);
  const discount    = p.eskom > 0 ? ((p.eskom - baseTariff) / p.eskom) * 100 : 0;
  const zone        = getZone(fraction, 100); // pass 100 so zone is purely based on coveragePct

  // ── Trajectory ────────────────────────────────────────────────────────────
  const traj = buildTrajectory(baseTariff, p.eskom, generated, 1, p.load, p.cpi, p.esEsc, term);

  // ── Monthly data ─────────────────────────────────────────────────────────
  // Scale base monthly supply by (coveragePct / defaultCov) to keep totals correct
      ...
      <div className="max-w-4xl mx-auto px-5">
        ...
        <section className="py-14">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            {/* Green Coverage card always matches the slider exactly */}
            <Stat label="Green Coverage"   value={`${coveragePct}%`}           sub={coveragePct===p.defaultCov ? 'contracted default' : coveragePct < p.defaultCov ? 'below contracted' : 'above contracted'} />
        ...
        <section className="py-14">
          ...
          <div className="bg-forest border border-border rounded-2xl p-5 mb-5">
            ...
            <div className="flex items-center justify-between mb-1">
              <div>
                ...
                <p className="text-[11px] text-dim mt-0.5">
                  Default = contracted coverage ({p.defaultCov}%). This number always matches the Green Coverage card above.
                </p>
              </div>
              <div className="flex items-center gap-3">
                ...
                {/* Large coverage % display — matches Green Coverage card */}
                <span className="text-offwhite font-mono font-bold text-xl">{coveragePct}%</span>
              </div>
            </div>

            {/* Slider — range 10%–150%, step 1% */}
            ...
            <div className="grid grid-cols-3 gap-3">
              {([
                { id:'A', label:'Zone A',    title:'Max Savings',   range:'10–99%',  desc:'Maximum cash flow. Every % of green coverage directly reduces cost.',   color:'#10B981' },
                { id:'B', label:'Zone B',    title:'Balanced',      range:'100–129%',desc:'Full green load coverage. Future-proofed against Eskom tariff hikes.',  color:'#34D399' },
                { id:'C', label:'Zone C ✦',  title:'Carbon Neutral',range:'130–150%',desc:'EU CBAM compliant. Enables premium green pricing in export markets.',   color:'#C9A84C' },
              ] as const).map(z => {
          ...
          {/* Live KPIs — all driven by coveragePct */}
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            {/* Green Coverage — always equals the slider value */}
            <div className="bg-green/10 border border-green/40 rounded-2xl p-4">
              ...
              <p className="text-xs text-dim">= slider value above</p>
            </div>
            <div className="bg-forest border border-border rounded-2xl p-4">
              ...
              <p className="text-xs text-dim">MWh / year at {coveragePct}%</p>
            </div>
            <div className="bg-forest border border-border rounded-2xl p-4">
        ...
        <section className="py-14">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {([5,10,15,20] as Term[]).map(t=>{
              ...
              const s   = raw * (coveragePct / p.defaultCov);
              const active = term===t;
              return (
                <button key={t} onClick={()=>setTerm(t)}
