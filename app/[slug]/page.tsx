...
const CARBON_TAX_ZAR          = CARBON_TAX_USD_PER_TON * ZAR_PER_USD; // R/ton
const ESKOM_WATER_L_PER_KWH   = 1.4;   // litres per kWh (coal intensity)
const TREES_PER_TON_CO2       = 45;    // trees to absorb 1 tCO2 per year
...
// ─── Savings trajectory (multiplier-aware) ────────────────────────────────────
type TRow = {
  year:             number;
  apolloTariff:     number;
  eskomTariff:      number;
...
function buildTrajectory(
  ...
  contractMwh: number, multiplier: number,
  customerLoad: number,
  ...
  return Array.from({ length: years }, (_, i) => {
    ...
    const effective     = Math.min(generated, customerLoad);   // can't credit more than consumed
    const saving        = (effective * e - generated * a) * 1000 / 1_000_000; // Mill ZAR
    cumul += saving;
    const row: TRow = {
      year:             i + 1,
...
// ─── 24-hour day profile (representative shape) ───────────────────────────────
const HOURS = Array.from({ length: 24 }, (_, h) => h);

function buildDayProfile(contractMwhPerYear: number, customerLoadMwh: number, multiplier: number) {
  // Apollo wheeled supply shape (solar + wind blend): ramps up 6am, peaks 12–14h, falls after 18h
  const apolloShape = [0, 0, 0, 0, 0, 0.05, 0.25, 0.55, 0.80, 0.95, 1.0, 1.0,
                       1.0, 0.95, 0.85, 0.70, 0.50, 0.30, 0.10, 0.05, 0, 0, 0, 0];
  // Customer load shape: low overnight, morning peak 8-9h, evening peak 17-20h
  const loadShape   = [0.35, 0.30, 0.28, 0.28, 0.30, 0.38, 0.55, 0.75, 0.85, 0.80, 0.78, 0.75,
                       0.72, 0.74, 0.76, 0.80, 0.90, 1.0, 0.98, 0.88, 0.72, 0.60, 0.48, 0.40];
  // Scale to MW (daily average MW)
  const apolloPeakMw = (contractMwhPerYear / 8760) * multiplier * 3.2;
  const loadPeakMw   = (customerLoadMwh / 8760) * 2.8;

  return HOURS.map((h, i) => {
    const apollo  = apolloShape[i] * apolloPeakMw;
    const load    = loadShape[i]   * loadPeakMw;
    const spill   = Math.max(0, apollo - load);
    return { hour: `${String(h).padStart(2,'0')}:00`, apollo: parseFloat(apollo.toFixed(3)), load: parseFloat(load.toFixed(3)), spill: parseFloat(spill.toFixed(3)) };
  });
}

// ─── Zone logic ───────────────────────────────────────────────────────────────
function getZone(multiplier: number, coverage: number): {
  id: 'A' | 'B' | 'C';
  label: string;
  sublabel: string;
  audience: string;
  ...
  const effective = coverage * multiplier;
  if (effective >= 90) return {
    id: 'C', label: 'Carbon Neutral', sublabel: 'EU CBAM Compliant',
    audience: 'Your Board & Export Customers',
    color: '#C9A84C', cbamCompliant: true,
  };
  if (effective >= 60) return {
    id: 'B', label: 'Balanced Growth', sublabel: 'Future-Proofed',
    audience: 'Your Operations Team',
    color: '#34D399', cbamCompliant: false,
  };
  return {
    id: 'A', label: 'Maximum Savings', sublabel: 'Cash Flow Optimised',
    audience: 'Your CFO',
    color: '#10B981', cbamCompliant: false,
  };
}
...
function ChartTip({ active, payload, label }: {
  ...
  return (
    <div className="bg-forest border border-border rounded-xl px-4 py-3 text-xs shadow-2xl z-50">
...
function Stat({ label, value, sub, accent = false }: {
  ...
  return (
    <div className={`rounded-2xl border p-5 flex flex-col gap-1 ${
      ...
      <p className={`text-2xl font-black leading-none ${accent ? 'text-green' : 'text-offwhite'}`}>{value}</p>
...
function SHead({ eye, title }: { eye: string; title: string }) {
  return (
    <div className="mb-8">
      ...
      <h2 className="font-display font-black text-offwhite leading-tight text-4xl md:text-5xl">{title}</h2>
    </div>
  );
}

function CCard({ title, children, gold = false }: {
...
function Logo() {
  return (
    <div className="flex items-end gap-2.5">
      {/* Try real logo first, fall back to SVG mark */}
      <div className="relative w-8 h-9 flex-shrink-0">
        <Image
          ...
          unoptimized
        />
        {/* SVG fallback always rendered behind — hidden if image loads */}
        <svg
...
// ─── CBAM / Carbon Tax card ───────────────────────────────────────────────────
function CBAMCard({
  contractMwh, multiplier, eskomTariff, zone,
}: {
  contractMwh: number; multiplier: number; eskomTariff: number; zone: ReturnType<typeof getZone>;
}) {
  const generated      = contractMwh * multiplier;
  const ghgPerYear     = generated * 0.94;            // tCO2e avoided
  const recs           = generated * multiplier;       // extra RECs when slider high
  const avoidedTax     = ghgPerYear * CARBON_TAX_ZAR / 1_000_000; // Mill ZAR/yr
  const gold           = zone.cbamCompliant;

  return (
    <div className={`rounded-2xl border p-6 transition-all duration-500 ${
      gold
        ? 'bg-gold/8 border-gold/50 shadow-xl shadow-gold/15'
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <span className={`inline-flex items-center gap-1.5 text-[11px] font-bold uppercase tracking-widest
          ...
          gold
            ? 'bg-gold/20 border-gold text-gold shadow-md shadow-gold/20'
            : 'bg-border/50 border-border text-muted'
        }`}>
          <span className={`w-1.5 h-1.5 rounded-full ${gold ? 'bg-gold animate-pulse' : 'bg-muted'}`} />
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mb-4">
        <div>
          <p className="text-[11px] font-semibold uppercase tracking-wider text-muted mb-1">GHG Avoided / yr</p>
        ...
        <div>
          <p className="text-[11px] font-semibold uppercase tracking-wider text-muted mb-1">Avoided Carbon Tax</p>
          <p className={`text-3xl font-black leading-none ${gold ? 'text-gold' : 'text-green'}`}>
            {fmtMill(avoidedTax)}
          </p>
        ...
        <div className="col-span-2 md:col-span-1">
          <p className="text-[11px] font-semibold uppercase tracking-wider text-muted mb-1">EU Carbon Price</p>
          <p className={`text-3xl font-black leading-none ${gold ? 'text-gold' : 'text-green'}`}>
      ...
      {gold && (
        <div className="mt-4 bg-gold/10 border border-gold/30 rounded-xl p-4">
          ...
          <p className="text-muted text-xs leading-relaxed">
            At {fmt(zone.id === 'C' ? 90 : 70, 0)}%+ renewable coverage your business qualifies for
            EU Carbon Border Adjustment Mechanism compliance — protecting exports and enabling
            premium green product pricing in European markets.
          </p>
        </div>
      )}

      <p className="text-border text-[11px] mt-3">
        Carbon price based on EU ETS Q1 2025 forward rate. Convert at R18.50/USD. For informational purposes.
      </p>
    </div>
  );
}

// ─── Environmental Legacy card ────────────────────────────────────────────────
function EnvLegacy({
  ...
  const items = [
    ...
    { icon: '🌳', label: 'Tree Equivalent', value: fmt(trees, 0), unit: 'trees planted', note: 'carbon absorption equiv.' },
  ...
  return (
    <div className="bg-forest border border-border rounded-2xl p-6">
      ...
      <p className="text-border text-[11px] mt-5 text-center">
        Water intensity: 1.4 L/kWh (Eskom coal avg). Carbon absorption: 45 trees/tCO₂e/yr. Vehicle avg 2.1 tCO₂e/yr.
      </p>
    </div>
  );
}
...
export default function ProposalPage({ params }: { params: { slug: string } }) {
  ...
  const [multiplier,  setMultiplier]  = useState(1.0);   // 0.8 → 1.8

  // ── Fetch
  useEffect(() => {
    supabase.from('proposals').select('*').eq('slug', params.slug).single()
      .then(({ data, error }) => {
  ...
  // ── Derived values (all reactive to term + multiplier) ──────────────────
  const baseTariff: number =
    term === 5  ? proposal.tariff_5yr  :
    term === 10 ? proposal.tariff_10yr :
    term === 15 ? proposal.tariff_15yr :
    /* 20 */      proposal.tariff_15yr * 0.97; // extrapolated 20yr discount

  const baseSavings: number =
    term === 5  ? proposal.savings_5yr  :
    term === 10 ? proposal.savings_10yr :
    term === 15 ? proposal.savings_15yr :
    /* 20 */      proposal.savings_15yr * 1.7;  // compound extrapolation
  ...
  // Multiplier-adjusted values
  const generated      = proposal.contract_mwh * multiplier;
  const effective      = Math.min(generated, proposal.customer_load_mwh);
  const spillageMwh    = Math.max(0, generated - proposal.customer_load_mwh);
  const spillagePct    = (spillageMwh / generated) * 100;
  const ghgAnnual      = generated * 0.94;
  const coverage       = (effective / proposal.customer_load_mwh) * 100;
  const adjustedSavings = baseSavings * multiplier * (effective / proposal.contract_mwh);
  const tariffDiscount  = ((proposal.eskom_tariff - baseTariff) / proposal.eskom_tariff) * 100;

  const zone = getZone(multiplier, proposal.green_coverage_pct);

  // ── Chart datasets
  const trajectory = useMemo(() => buildTrajectory(
    baseTariff, proposal.eskom_tariff,
    proposal.contract_mwh, multiplier, proposal.customer_load_mwh,
    proposal.escalation_cpi, proposal.eskom_escalation, term
  ), [baseTariff, proposal.eskom_tariff, proposal.contract_mwh, multiplier,
      proposal.customer_load_mwh, proposal.escalation_cpi, proposal.eskom_escalation, term]);

  const monthly = MONTH_LABELS.map((label, i) => {
    const supply = (proposal.monthly_supply as Record<string,number>)[MONTH_KEYS[i]] ?? 0;
    const load   = (proposal.monthly_load   as Record<string,number>)[MONTH_KEYS[i]] ?? 0;
    const adj    = supply * multiplier;
    return {
      month:  label,
      supply: parseFloat(adj.toFixed(0)),
      load,
      spill:  parseFloat(Math.max(0, adj - load).toFixed(0)),
    };
  });

  const dayProfile = useMemo(() =>
    buildDayProfile(proposal.contract_mwh, proposal.customer_load_mwh, multiplier),
    [proposal.contract_mwh, proposal.customer_load_mwh, multiplier]
  );
  ...
  const tariffBars = [
    ...
    { term: '20yr', apollo: proposal.tariff_15yr * 0.97, eskom: proposal.eskom_tariff },
  ];

  const legendFmt = (v: string) => <span className="text-muted text-xs">{v}</span>;
  ...
  return (
    <div className="min-h-screen bg-charcoal text-offwhite font-sans">
      ...
      <div className="sticky top-0 z-50 border-b border-border bg-charcoal/95 backdrop-blur-sm">
        <div className="max-w-5xl mx-auto px-5 py-3 flex flex-col sm:flex-row items-start sm:items-center gap-3">
          {/* Term toggle */}
          <div className="flex items-center gap-1 bg-forest border border-border rounded-xl p-1 flex-shrink-0">
          ...
          {/* Zone badge */}
          <div
            className="hidden sm:flex items-center gap-2 px-3 py-1.5 rounded-lg border text-xs font-bold uppercase tracking-widest transition-all duration-500"
            style={{ borderColor: zone.color + '60', background: zone.color + '15', color: zone.color }}
          >
      ...
      <div className="max-w-5xl mx-auto px-5">
        ...
        <section className="py-16">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-3">
            <Stat label="Contracted Supply"          value={`${fmt(proposal.contract_mwh, 0)} MWh`} sub="base per year"          accent />
            <Stat label={`${term}-yr Weighted Tariff`} value={`R${fmt(baseTariff)} /kWh`}         sub={`${fmt(tariffDiscount,1)}% below Eskom`} />
        ...
        {/* ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
            GREEN VOLUME SLIDER — THE STRATEGIC ENGINE
        ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━ */}
        <section className="py-16">
          ...
          <div className="bg-forest border border-border rounded-2xl p-6 mb-6">
            <div className="flex items-center justify-between mb-3">
              ...
              <div className="flex items-center gap-3">
                ...
                <span className="text-offwhite font-mono font-bold text-lg">{fmt(multiplier * 100, 0)}%</span>
              </div>
            </div>
            ...
            <div className="relative mb-4">
              <input
                ...
                style={{
                  background: `linear-gradient(to right, ${zone.color} 0%, ${zone.color} ${(multiplier - 0.8) / 1.0 * 100}%, #1E4D30 ${(multiplier - 0.8) / 1.0 * 100}%, #1E4D30 100%)`,
                }}
              />
              {/* Zone labels */}
              <div className="flex justify-between mt-2 text-[10px] font-semibold uppercase tracking-wide">
            ...
            <div className="grid grid-cols-3 gap-3 mt-4">
              {[
                { id: 'A', label: 'Zone A',     title: 'Max Savings',    range: '80–99%',  audience: 'CFO Focus',        desc: 'Maximum cash flow. Lowest tariff. Ideal for cost reduction mandates.',  color: '#10B981' },
                { id: 'B', label: 'Zone B',     title: 'Balanced',       range: '100–129%',audience: 'Operations Focus', desc: 'Future-proofed. Balanced between cost and green coverage.',              color: '#34D399' },
                { id: 'C', label: 'Zone C ✦',   title: 'Carbon Neutral', range: '130–180%',audience: 'Board & Export',   desc: 'EU CBAM compliant. Enables green premium pricing in export markets.',   color: '#C9A84C' },
              ].map(z => {
                ...
                return (
                  <div key={z.id}
                    ...
                    <p className="text-muted text-[10px] mt-1 leading-tight">{z.desc}</p>
                    <p className="text-[10px] font-semibold mt-2" style={{ color: z.color }}>{z.audience}</p>
                  </div>
                );
              })}
            </div>
          ...
          {/* Live KPIs that respond to slider */}
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            ...
            <div className={`rounded-2xl border p-5 transition-all duration-300 ${
              ...
              <p className="text-[11px] font-semibold uppercase tracking-widest text-muted mb-1">Energy Spillage</p>
        ...
        <section className="py-16">
          ...
          <CBAMCard
            contractMwh={proposal.contract_mwh}
            multiplier={multiplier}
            eskomTariff={proposal.eskom_tariff}
            zone={zone}
          />
        </section>
          ...
          <CCard title="Apollo Wheeled Supply vs Electrical Load [MWh / month]">
            <ResponsiveContainer width="100%" height={280}>
              <AreaChart data={monthly} margin={{ top: 8, right: 8, left: 0, bottom: 0 }}>
                ...
                {spillageMwh > 0 && (
                  <Area type="monotone" dataKey="spill" name="Spillage (excess)" stroke="#C9A84C" strokeWidth={1.5} fill="url(#spg)" dot={false} strokeDasharray="4 2"/>
                )}
          ...
          <div className="mt-5">
            <CCard title="Day-in-the-Life Match — 24-Hour Cycle [MW average]">
              <p className="text-muted text-xs mb-4 -mt-2">
                Representative daily supply/demand profile. Move the Green Volume Slider to see how Apollo supply covers your load.
              </p>
              <ResponsiveContainer width="100%" height={240}>
                <AreaChart data={dayProfile} margin={{ top: 8, right: 8, left: 0, bottom: 0 }}>
                  ...
                  {spillageMwh > 0 && (
                    <Area type="monotone" dataKey="spill" name="Spillage" stroke="#C9A84C" strokeWidth={1.5} fill="url(#dsg)" dot={false} strokeDasharray="4 2"/>
                  )}
                  <Area type="monotone" dataKey="apollo" name="Apollo Supply (MW)" stroke="#10B981" strokeWidth={2.5} fill="url(#dag)" dot={false}/>
                  <Line type="monotone" dataKey="load"   name="Eskom Load (MW)"   stroke="#F0FFF4" strokeWidth={2} dot={false} strokeDasharray="5 3"/>
                  <ReferenceLine y={0} stroke="#1E4D30"/>
                </AreaChart>
              </ResponsiveContainer>
              <p className="text-border text-[11px] mt-3">
                Indicative shape based on wind/solar blend. Actual output varies by weather and grid conditions.
                Spill (gold) appears when Apollo supply exceeds consumption — visible above the load line.
              </p>
            </CCard>
          </div>
        ...
        <section className="py-16">
          ...
          {/* Term picker — 4 options including 20yr */}
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {([5, 10, 15, 20] as Term[]).map(t => {
              const s =
                t === 5  ? proposal.savings_5yr  :
                t === 10 ? proposal.savings_10yr :
                t === 15 ? proposal.savings_15yr :
                           proposal.savings_15yr * 1.7;
              const active = term === t;
              return (
                <button key={t} onClick={() => setTerm(t)}
                  ...
                  <p className="text-green text-3xl font-black mt-2">{fmtMill(s * multiplier)}</p>
        ...
        <section className="py-16">
          ...
          <div className="grid md:grid-cols-2 gap-5 mb-5">
            ...
            <CCard title="Tariff Schedule [R/kWh] — 1 April 2025">
              <table className="w-full text-xs border-collapse">
                ...
                <tbody>
                  {[
                    { label: 'Weighted Avg', vals: [proposal.tariff_5yr, proposal.tariff_10yr, proposal.tariff_15yr, proposal.tariff_15yr*0.97, proposal.eskom_tariff], bold: true },
                    ...
                    <tr key={row.label}
                      ...
                      {row.vals.map((v, j) => (
                        <td key={j} className={`text-center py-2.5 ${j === 4 ? 'text-danger' : 'text-offwhite'} ${row.bold ? 'font-bold' : ''}`}>
                          {fmt(v)}
                        </td>
                      ))}
                    </tr>
          ...
          <CCard title={`Tariff Trajectory [R/kWh] — ${term}-Year View`}>
            ...
            <p className="text-dim text-xs mt-3">
              Apollo escalates at CPI ({proposal.escalation_cpi}% p.a.) vs Eskom at {proposal.eskom_escalation}% p.a.
              The divergence accelerates your savings every year.
            </p>
          </CCard>
        </section>
